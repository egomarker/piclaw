...
const ALLOWED_UNUSED_EXPORTS = new Map<string, string>([
  ...
  ["src/addons/runtime-contributions.ts - getAddonNonWebCommandPolicies", "compatibility re-export for add-on manifest policy consumers"],
  ["src/addons/runtime-contributions.ts - getAddonRecoveryExcludedChatJidPrefixes", "compatibility re-export for add-on recovery policy consumers"],
  ["src/addons/runtime-contributions.ts - getInstalledAddonRuntimeEntryPaths", "compatibility re-export for legacy add-on runtime discovery consumers"],
  ...
  ["src/agent-pool/automatic-recovery.ts - DEFAULT_AUTOMATIC_RECOVERY_CONFIG", "recovery policy fixture retained for focused automatic-recovery regression tests"],
  ["src/core/domain-config.ts - boolField", "domain-config framework constructor retained for #752+ domain migration tranches"],
  ["src/core/domain-config.ts - clearDomainConfigRegistryForTests", "test-only API for focused core/domain-config.ts regression harnesses"],
  ["src/core/domain-config.ts - getRegisteredDomainConfigSchemas", "domain-config schema inspection retained for future settings surfaces and tests"],
  ["src/core/domain-config.ts - integerField", "domain-config framework constructor retained for #752+ domain migration tranches"],
  ["src/core/domain-config.ts - migrateDomainConfig", "domain-config migration API retained for #752+ domain migration tranches"],
  ["src/core/domain-config.ts - registerDomainConfig", "domain-config registration API retained for #752+ domain migration tranches"],
  ["src/core/domain-config.ts - resetDomainConfigWarningsForTests", "test-only API for focused core/domain-config.ts compatibility-warning regression harnesses"],
  ["src/core/domain-config.ts - stringField", "domain-config framework constructor retained for #752+ domain migration tranches"],
  ...
  ["src/agent-pool/compaction.ts - setCompactionSettlementGraceForTests", "test-only API for bounded compaction settlement and generation-race coverage"],
  ...
  ["src/dream.ts - DREAM_TASK_CRON", "Dream scheduler compatibility export retained for fresh-process configuration tests"],
  ...
  ["src/db.ts - getNewMessages", "public database barrel contract retained for runtime modules, add-ons, and tests importing ./db.js"],
  ...
  ["src/runtime/channel-transport-registry.ts - resetChannelTransportRegistryForTests", "test-only API for focused runtime channel transport regression harnesses"],
  ["src/runtime/non-web-command-policy.ts - getBlockedNonWebCommandMessage", "non-web command policy helper retained for channel integrations and focused tests"],
