...
- **Provider-native compaction** opt-in pre-pass and timeout. Shared source preparation and tool analysis run first; a successful supported remote attempt then completes before the local method's ledger/prompt/model-execution path, while safe failure continues into the captured Selective or Pipelined fallback. Manual `/compact` reports expose only a Piclaw-marked readable checkpoint when present, never encrypted or arbitrary provider output, and refresh the context indicator immediately from rebuilt-session or safety-adjusted report estimates. See [Provider-native remote compaction](configuration.md#provider-native-remote-compaction).
